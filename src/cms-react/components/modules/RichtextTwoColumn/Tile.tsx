...
const Tile = ({ data }: TileProps) => {
	...
	return (
        <div className={`${data.bodyOptions.bodyBackgroundColor} h-full flex flex-col p-4 md:p-12 `}>
            <div className={`prose ${data.bodyOptions.bodyBackgroundColor == 'bg-white' ? 'text-black' : 'text-white'} prose-black text-black prose-a:text-orange-500 prose-p:mt-0 prose-a:no-underline max-w-full [&_h2]:mb-5 [&_h2]:mt-0 [&_h2]:font-bold [&_h2]:text-3xl [&_h2]:text-sky-500`} dangerouslySetInnerHTML={{__html: data.bodyOptions?.bodyDetails}}></div>
